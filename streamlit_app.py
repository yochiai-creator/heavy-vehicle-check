...
menu_options = ["点検入力", "管理者メニュー", "エラー検知", "異常一覧", "履歴・出力", "ログ整理", "車両マスター", "点検者マスター", "QRコード発行", "QR印刷台紙"]
    ...
    if require_admin():
        ...
        if unchecked.empty:
            ...
            for _, r in unchecked.iterrows():
                st.error(f"{r['vehicle_no']} / {r['vehicle_name']} / {VEHICLE_TYPES.get(r['vehicle_type'], r['vehicle_type'])}")
        ...
        if vehicles_locked.empty:
            ...
            for _, r in vehicles_locked.iterrows():
                st.error(f"{r['vehicle_no']} / {r['vehicle_name']}")
    ...
    if require_admin():
        ...
        if df.empty:
            ...
            new_type = st.selectbox(
                "区分",
                EDIT_TYPE_KEYS,
                index=EDIT_TYPE_KEYS.index(current_type),
