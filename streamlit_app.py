...
def update_vehicle_active(vehicle_no, active):
    con = connect()
    con.execute("UPDATE vehicles SET active = ? WHERE vehicle_no = ?", (1 if active else 0, vehicle_no))
    con.commit()
    con.close()
...
def save_inspection(vehicle, inspector, meter, statuses, abnormal_detail, action_detail, photo):
    ...
    if has_abnormal:
        cur.execute(
            "UPDATE vehicles SET use_locked = 1 WHERE vehicle_no = ?",
            (vehicle["vehicle_no"],),
        )
...
st.caption("Streamlit版 / QR車両選択 / SQLite保存 / 写真DB保存 / 管理者確認 / CSV出力")

menu = st.sidebar.radio(
    "メニュー",
    ["点検入力", "異常一覧", "履歴・出力", "車両マスター", "QRコード発行", "バックアップ"],
)
...
if menu == "点検入力":
    ...
    if query_vehicle in options:
        ...
        st.info(f"QRコードから車両を自動選択しました：{query_vehicle}")
    ...
    if not df.empty:
        target = st.selectbox("有効/無効を切り替える車両", df["vehicle_no"].tolist())
        active = st.radio("状態", ["有効", "無効"], horizontal=True)
        if st.button("状態を更新"):
            update_vehicle_active(target, active == "有効")
            st.success("更新しました。")
            st.rerun()
    ...
    st.info("ここで車両ごとのQRコードを作れます。QRを読むと該当車両が自動選択されます。")

    base_url = st.text_input(
        "アプリURL",
        placeholder="例：https://heavy-vehicle-check-xxxxx.streamlit.app",
        help="今開いているStreamlitアプリのURLをコピーして貼ってください。",
    )
    ...
    if vehicles.empty:
        ...
        if st.button("QRコード作成", type="primary", use_container_width=True):
            if not base_url:
                st.warning("アプリURLを入力してください。")
            else:
                url = base_url.rstrip("/") + f"/?vehicle={selected_no}"
                png = make_qr_png(url)
