...
APP_URL = st.secrets.get("APP_URL", "https://heavy-vehicle-check-ghka28mxhavp4qrjpnkb7b.streamlit.app")
ADMIN_CODE = st.secrets.get("ADMIN_CODE", "1224")
...
def migrate_db():
    ...
    cur = con.cursor()
    try:
        cur.execute("PRAGMA table_info(inspectors)")
        cols = [r[1] for r in cur.fetchall()]
        if "department" not in cols:
            cur.execute("ALTER TABLE inspectors ADD COLUMN department TEXT")
        con.commit()
    finally:
        con.close()
...
def get_inspectors(active_only=True):
    ...
    con.close()
    if not df.empty:
        for col in ["inspector_name", "department", "note"]:
            if col in df.columns:
                df[col] = df[col].apply(clean_text)
    return df

def add_inspector(name, department, note, certs):
    cert_data = []
...
def update_inspector(inspector_id, name, department, active, note, certs=None, replace_certs=False):
    con = connect()
    if replace_certs:
        cert_data = []
        for cert in (certs or [])[:4]:
        ...
        con.execute(
            """UPDATE inspectors
               SET inspector_name=?, department=?, active=?, note=?,
               ...
               WHERE id=?""",
            (
                clean_text(name), clean_text(department), 1 if active else 0, clean_text(note),
                cert_data[0][0], cert_data[0][1],
                cert_data[1][0], cert_data[1][1],
                cert_data[2][0], cert_data[2][1],
                cert_data[3][0], cert_data[3][1],
        ...
        con.execute(
            "UPDATE inspectors SET inspector_name=?, department=?, active=?, note=? WHERE id=?",
            (clean_text(name), clean_text(department), 1 if active else 0, clean_text(note), inspector_id),
        )
    con.commit()
    con.close()
...
def show_save_party():
    ...
    .party-box {
        ...
        font-size: 72px;
        padding: 18px 0;
        animation: partyPop 0.8s ease-in-out;
    }
    @keyframes partyPop {
...
if menu == "点検入力":
    ...
    if inspectors.empty:
        ...
        inspector_options = inspectors["inspector_name"].tolist()
        inspector = st.selectbox(
            "点検者",
            inspector_options,
            format_func=lambda n: f"{n} / {clean_text(inspectors.loc[inspectors['inspector_name'] == n, 'department'].iloc[0]) or '部署未設定'}",
        )
    ...
    if require_admin():
        ...
        c1, c2 = st.columns(2)
        start = c1.date_input("削除開始日", value=date.today() - timedelta(days=RETENTION_DAYS + 30))
        end = c2.date_input("削除終了日", value=date.today() - timedelta(days=RETENTION_DAYS))
    ...
    if require_admin():
        with st.form("add_inspector_form"):
            ...
            department = st.text_input("部署名", placeholder="例：製造部 / 仕上げ検査B班 / 土木部")
            note = st.text_area("備考")
            ...
            if st.form_submit_button("登録", use_container_width=True):
                if not clean_text(name):
                    ...
                    if certs and len(certs) > 4:
                        certs = certs[:4]
                    ok, msg = add_inspector(name, department, note, certs)
                    if ok:
                        st.success(msg)
                        st.rerun()
                    else:
        ...
        if df.empty:
            ...
            for _, row in df.iterrows():
                with st.container(border=True):
                    c1, c2, c3 = st.columns(3)
                    c1.write(f"**点検者名**\n{row['inspector_name']}")
                    c2.write(f"**部署名**\n{clean_text(row.get('department', '')) or '未設定'}")
                    c3.write(f"**状態**\n{'有効' if row['active'] else '無効'}")
                    if clean_text(row.get("note")):
                        st.write(f"備考：{clean_text(row['note'])}")
                    render_certs(row)

                    with st.expander("この点検者を編集"):
                        edit_name = st.text_input("点検者名", value=row["inspector_name"], key=f"edit_name_{row['id']}")
                        edit_department = st.text_input("部署名", value=clean_text(row.get("department", "")), key=f"edit_department_{row['id']}")
                        edit_note = st.text_area("備考", value=clean_text(row.get("note", "")), key=f"edit_note_{row['id']}")
                        edit_active = st.radio(
                            "状態",
                            ["有効", "無効"],
                            index=0 if row["active"] else 1,
                            horizontal=True,
                            key=f"edit_active_{row['id']}",
                        )
                        replace_certs = st.checkbox("資格者証を差し替える", key=f"replace_certs_{row['id']}")
                        new_certs = []
                        if replace_certs:
                            new_certs = st.file_uploader(
                                ...
                                key=f"new_certs_{row['id']}",
                            )
                            if new_certs and len(new_certs) > 4:
                                new_certs = new_certs[:4]

                        if st.button("編集内容を保存", key=f"save_inspector_{row['id']}", use_container_width=True):
                            if not clean_text(edit_name):
                                st.warning("点検者名を入力してください。")
                            else:
                                try:
                                    update_inspector(
                                    ...
                                    st.success("点検者情報を更新しました。")
                                    st.rerun()
                                except sqlite3.IntegrityError:
                                    st.error("同じ点検者名が既にあります。")

                    with st.expander("この点検者を削除"):
                        del_code = st.text_input("削除用 管理者コード", type="password", key=f"delete_inspector_code_{row['id']}")
                        if st.button("削除実行", key=f"delete_inspector_{row['id']}", use_container_width=True):
                            if del_code != ADMIN_CODE:
                                st.error("管理者コードが違います。")
                            else:
