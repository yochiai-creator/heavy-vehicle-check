...
def add_vehicle(vehicle_no, vehicle_name, vehicle_type, next_inspection_date, note):
    ...
    try:
        con.execute(
            ...
            (vehicle_no, vehicle_name, vehicle_type, datetime.now().strftime("%Y-%m-%d %H:%M:%S"), next_inspection_date.isoformat(), note),
        )
        con.commit()
...
st.title("🚜 野田組 車両点検記録")
st.caption("管理者QR / 車両固定QR / 写真4枚 / 点検忘れ一覧 / 西暦日付 / Excel出力")
...
if menu == "点検入力":
    ...
    if require_admin():
        ...
        st.subheader("本日未点検一覧")
        if unchecked.empty:
            st.success("本日の未点検車両はありません。")
        else:
            jp = unchecked.rename(columns={"vehicle_no": "車両番号", "vehicle_name": "車両名", "vehicle_type": "車種", "use_locked": "使用禁止"})
            jp["車種"] = jp["車種"].map(VEHICLE_TYPES)
            st.table(jp)

        st.subheader("管理者QR")
        st.image(make_qr_png(admin_url()), caption=admin_url(), width=260)
        st.download_button("管理者QRダウンロード", data=make_qr_png(admin_url()), file_name="管理者QR.png", mime="image/png")
    ...
    if require_admin():
        ...
        if df.empty:
            ...
            export_df = export_df.rename(columns={
                ...
                "vehicle_no": "車両番号", "vehicle_name": "車両名", "vehicle_type": "車種",
            ...
            if "車種" in export_df.columns:
                export_df["車種"] = export_df["車種"].map(VEHICLE_TYPES)
    ...
    if require_admin():
        ...
        if df.empty:
            ...
            new_type = st.selectbox("車種", list(VEHICLE_TYPES.keys()), index=list(VEHICLE_TYPES.keys()).index(row["vehicle_type"]), format_func=lambda x: VEHICLE_TYPES[x])
