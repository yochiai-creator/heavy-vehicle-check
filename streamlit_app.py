...
EDIT_TYPE_KEYS = ["forklift", "vehicle", "construction", "other"]
...
def seed_vehicles():
    ...
    if cur.fetchone()[0] == 0:
        ...
        for no, name, typ in rows:
            cur.execute("""INSERT INTO vehicles(vehicle_no, vehicle_name, vehicle_type, active, use_locked, created_at, next_inspection_date, note)
                           VALUES (?, ?, ?, 1, 0, ?, ?, '')""", (no, name, typ, now, next_d))
    con.commit()
    con.close()
...
def add_vehicle(no, name, typ, next_d, note):
    con = connect()
    try:
        con.execute("""INSERT INTO vehicles(vehicle_no, vehicle_name, vehicle_type, active, use_locked, created_at, next_inspection_date, note)
                       VALUES (?, ?, ?, 1, 0, ?, ?, ?)""",
                    (no.strip(), name.strip(), typ, datetime.now().strftime("%Y-%m-%d %H:%M:%S"), next_d.isoformat(), note))
        con.commit()
...
def update_vehicle(no, name, typ, active, next_d, note):
    con = connect()
    con.execute("""UPDATE vehicles SET vehicle_name=?, vehicle_type=?, active=?, next_inspection_date=?, note=? WHERE vehicle_no=?""",
                (name, typ, 1 if active else 0, next_d.isoformat(), note, no))
    con.commit()
    con.close()
...
def get_items(inspection_id):
    con = connect()
    df = pd.read_sql_query("SELECT item_name, status FROM inspection_items WHERE inspection_id=?", con, params=(inspection_id,))
    con.close()
    return df
...
def render_photos(row, width=160):
    pairs = [("photo_name","photo_bytes"),("photo2_name","photo2_bytes"),("photo3_name","photo3_bytes"),("photo4_name","photo4_bytes")]
    cols = st.columns(4)
    shown = False
...
if menu == "点検入力":
    ...
    if query_vehicle and query_vehicle in options:
        ...
        selected_no = st.selectbox("車両", options, format_func=lambda no: f"{no} / {vehicles.loc[vehicles['vehicle_no']==no, 'vehicle_name'].iloc[0]}")
    ...
    for i, item in enumerate(CHECK_ITEMS.get(vehicle_type, CHECK_ITEMS["other"])):
        with st.container(border=True):
            st.markdown(f"**{item}**")
            statuses[item] = st.radio("判定", ["良好", "要整備（使用可）", "異常あり", "対象外"], horizontal=True, key=f"{selected_no}_{i}_{item}")
    ...
    if require_admin():
        ...
        if unchecked.empty:
            ...
            for _, r in unchecked.iterrows():
                st.write(f"{r['vehicle_no']} / {r['vehicle_name']} / {VEHICLE_TYPES.get(r['vehicle_type'], r['vehicle_type'])}")
    ...
    if require_admin():
        ...
        if df.empty:
            ...
            export_df = export_df.rename(columns={
                "id":"ID","inspected_at":"保存日時","inspection_date":"点検日","vehicle_no":"車両番号",
                "vehicle_name":"車両名","vehicle_type":"区分","inspector":"点検者","meter":"メーター",
                "result":"判定","abnormal_detail":"異常内容","action_detail":"対応内容",
    ...
    if require_admin():
        ...
        if df.empty:
            ...
            for _, r in df.iterrows():
                with st.container(border=True):
                    c1, c2, c3, c4 = st.columns(4)
                    c1.markdown(f'**車両番号**<br>{str(r["vehicle_no"]).replace("\\n","")}', unsafe_allow_html=True)
                    c2.markdown(f'**車両名**<br>{str(r["vehicle_name"]).replace("\\n","")}', unsafe_allow_html=True)
                    c3.markdown(f'**区分**<br>{VEHICLE_TYPES.get(r["vehicle_type"], r["vehicle_type"])}', unsafe_allow_html=True)
                    c4.markdown(f'**状態**<br>{"有効" if r["active"] else "無効"}', unsafe_allow_html=True)
                    ...
                    if r.get("use_locked",0):
                        st.error("使用禁止中")
                    if r.get("note"):
                        note_val='' if str(r['note']).lower()=='nan' else str(r['note']).replace('\\n',' ')
                        if note_val:
                            st.write(f'備考：{note_val}')
            ...
            current_type = row["vehicle_type"] if row["vehicle_type"] in EDIT_TYPE_KEYS else "other"
            new_type = st.selectbox("区分", EDIT_TYPE_KEYS, index=EDIT_TYPE_KEYS.index(current_type), format_func=lambda x: VEHICLE_TYPES[x])
